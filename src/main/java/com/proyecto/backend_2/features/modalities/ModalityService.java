...
public class ModalityService {
    ...
    public List<ModalityModel> get() {
        return repository.findAll();
    }

    public List<ModalityModel> getByState(Integer state) {
        if (state == 2) {
            return repository.findAll();
        }
    ...
    public ResponseEntity<ApiResponse> post(ModalityModel post) {
        ...
        ModalityModel model = ModalityModel.builder()
    ...
    public ResponseEntity<ApiResponse> put(Integer id, ModalityModel put) {
        ModalityModel existing = repository.findById(id)
        ...
        put.setCodm(id);
        ModalityModel modify = repository.save(put);
