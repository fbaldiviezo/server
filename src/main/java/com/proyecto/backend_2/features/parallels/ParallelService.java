...
public class ParallelService {
    private final ParallelRepository repository;
    private final CustomResponseBuilder customResponseBuilder;

    public List<ParallelModel> get() {
        return repository.findAll();
    }

    public List<ParallelModel> getByState(Integer state) {
        if (state == 2) {
            return repository.findAll();
        }
    ...
    public List<ParallelsBySubjectDto> getParallelsBySubject(String codmat) {
        return repository.getParallelsBySubject(codmat);
    }

    public ResponseEntity<ApiResponse> post(ParallelModel post) {
        ...
        ParallelModel model = ParallelModel.builder()
    ...
    public ResponseEntity<ApiResponse> put(Integer id, ParallelModel put) {
        ParallelModel existing = repository.findById(id)
        ...
        put.setCodpar(id);
        ParallelModel modify = repository.save(put);
